...
ps.querySelectorAll('.mtk-pswitch__item').forEach(item => {
  item.addEventListener('click', () => {
    ps.querySelectorAll('.mtk-pswitch__item').forEach(i => i.classList.remove('active'));
    item.classList.add('active');
    if (stateEl) stateEl.textContent = `Active: ${item.querySelector('.mtk-pswitch__name').textContent}`;
